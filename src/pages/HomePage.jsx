    ...
    <div>
      <section className="relative bg-gradient-to-br from-blue-700 via-blue-600 to-blue-500 text-white overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-10">
          <div className="flex-1 text-center md:text-left">
            <div className="inline-block bg-white/20 rounded-full px-3 py-1 text-xs font-medium mb-4 tracking-wide uppercase">MediConnect Clinic</div>
          ...
          <div className="w-52 h-52 rounded-3xl bg-white/20 flex items-center justify-center text-8xl shadow-2xl border-4 border-white/30 shrink-0 z-10">🩺</div>
        </div>
      ...
      <section className="bg-white border-b border-slate-100">
        <div className="max-w-5xl mx-auto px-6 py-6 grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          {[["15+", "Years Experience"], ["12,000+", "Patients Treated"], ["4.9★", "Average Rating"], ["6", "Specialities"]].map(([v, l]) => (
            <div key={l}><p className="text-2xl font-bold text-blue-600">{v}</p><p className="text-xs text-slate-500">{l}</p></div>
          ))}
        </div>
      </section>
